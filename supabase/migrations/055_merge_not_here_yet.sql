-- Remove the persisted soft-reset attendance state. "Not here yet" is now
-- represented only by the absence of an attendance_records row.

-- DELETE is allowed only through the clear RPC/offline/retrospective paths.
...
DECLARE
    ...
    v_clear BOOLEAN := COALESCE(
        current_setting('app.attendance_clear', TRUE), 'off'
    ) = 'on';
BEGIN
    IF COALESCE(current_setting('app.suppress_audit', TRUE), 'off') = 'on' THEN
        RETURN COALESCE(NEW, OLD);
    END IF;
    IF TG_OP = 'DELETE' AND NOT v_clear THEN
...
-- Study Space now stores only Present; clearing deletes the row.
...
-- Preserve the mutation being replaced so a delayed retry cannot recreate a
-- row after a newer clear.
CREATE OR REPLACE FUNCTION public.archive_deleted_attendance_receipt()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, pg_temp
AS $$
BEGIN
    IF OLD.client_mutation_id IS NOT NULL THEN
...
REVOKE EXECUTE ON FUNCTION public.archive_deleted_attendance_receipt()
    FROM PUBLIC, anon, authenticated;

DROP TRIGGER IF EXISTS archive_deleted_attendance_receipt ON attendance_records;
CREATE TRIGGER archive_deleted_attendance_receipt
AFTER DELETE ON attendance_records
FOR EACH ROW EXECUTE FUNCTION public.archive_deleted_attendance_receipt();

-- Idempotent, actor-bound clear. Context flags set by sync/retrospective calls
-- expand the date window; direct calls are restricted to today's open session.
...
-- A null status is an offline clear; concrete writes accept three values only.
...
-- Retrospective Unmarked now clears the persisted row.
...
-- Delete the retired facts before tightening the constraint. The audit trigger
-- records each deletion; immutable migration/audit history is intentionally kept.
SELECT set_config('app.attendance_clear', 'on', TRUE);
SELECT set_config('app.retrospective_attendance_write', 'on', TRUE);
DELETE FROM attendance_records WHERE status = 'excused';
SELECT set_config('app.attendance_clear', 'off', TRUE);
...
RETURNS TABLE (
    ...
    total_count BIGINT, on_time_rate NUMERIC
)
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
-- Verification: data, accepted domain, clear API, and reporting shape.
DO $$
DECLARE
    v_constraint TEXT;
BEGIN
    ...
    ), 'retired attendance rows remain';
    ASSERT v_constraint NOT ILIKE '%excused%',
           'attendance status constraint still accepts retired value';
    ASSERT EXISTS (
        SELECT 1 FROM pg_proc
        WHERE oid = 'public.clear_attendance(uuid,uuid,text)'::REGPROCEDURE
    ), 'clear_attendance missing';
    ASSERT NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_schema = 'public'
          AND table_name = 'attendance_summary'
          AND column_name = 'excused_count'
    ), 'attendance_summary still exposes retired count';
    ASSERT (

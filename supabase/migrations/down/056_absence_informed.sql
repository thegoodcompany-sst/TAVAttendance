-- Reverse of 056_absence_informed.sql.
-- Restores roster RPC shapes and mark_retrospective_attendance to the
-- three-argument form from migration 055. Drops the companion column.
...
DROP FUNCTION IF EXISTS public.mark_retrospective_attendance(UUID, UUID, TEXT, BOOLEAN);
...
-- Restore sync_attendance without absence_informed (055 body).
...
-- Restore integrity trigger without absence_informed nulling (055 body).
...
ALTER TABLE attendance_records
    DROP COLUMN IF EXISTS absence_informed;

-- 058_offline_observed_cas.sql
-- Offline sync CAS: an older unprocessed mutation must not overwrite a newer
-- authorised correction. Server does not trust client wall clock.
--
-- Optional per-record JSON key observed_marked_at:
--   key absent  → legacy apply (1.1.3 clients)
--   JSON null   → client observed no row (Not Here Yet)
--   timestamptz → client observed that exact attendance_records.marked_at

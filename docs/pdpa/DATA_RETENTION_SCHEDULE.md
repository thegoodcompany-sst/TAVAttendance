...
| Exam result slips and student photos | Deleted on explicit app-driven erasure; scheduled-retention file cleanup pending | App clients sweep Storage before `anonymise_student` / `erase_student`; see HUMANS.md §9 |
...
- **On-demand erasure** (e.g. a valid erasure request): an admin uses the in-app "Erase student"
  action → the client deletes the student's `result-slips` and `student-photos`
  Storage objects, then calls `erase_student()` (hard delete + audit scrub) or
  `anonymise_student()`.
...
- The database-only daily purge cannot delete Storage objects. A server-side
  orphan cleanup remains required for scheduled retention (HUMANS.md §9).
- Review this schedule annually with the DPO.

_Last updated: 2026-07-16 (draft)._

...
Applied 2026-07-29 via linked `supabase db query -f` (not `db push`; sparse
ledger). Verified function bodies for study-space roster, sync staff gate,
parent-link role, and device-token ownership; ran cleaned
`scripts/prod-security-check.sql` against prod (asserts pass); `NOTIFY pgrst,
'reload schema'`.
